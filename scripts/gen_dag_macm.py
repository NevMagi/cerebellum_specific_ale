...
def generate_dagman_file():
    ...
    zmap_paths = glob.glob('/data/project/cerebellum_ale/output/SALE_merged/*/*/corr_cluster_h-001_k-50_mask-D2009_MNI_z.nii.gz')
    job_count = 0
    for zmap_path in zmap_paths:
        zmap = nibabel.load(zmap_path)
        sig = np.any(zmap.get_fdata().flatten()>0)
        if sig:
            f.write(f'JOB job_{job_count} /data/project/cerebellum_ale/scripts/run_macm.submit\n')
