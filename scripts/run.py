...
from utils import filter_coords_to_mask, get_null_xyz, cluster_extent_correction, merge_experiments
...
def prepare_data(bd_prefix, pure=False, merge_exps=True, out_path=None):
    ...
    if merge_exps:
        # merge contrasts (experiments) within studies
        dset = merge_experiments(dset)
...
def sale_k_cluster(k=50, height=0.001, mask_name='D2009_MNI'):
    ...
    uncorr_paths = glob.glob('/data/project/cerebellum_ale/output/SALE_merged/*/*/uncorr.pkl.gz') \
        + glob.glob('/data/project/cerebellum_ale/output/SALE_merged/*/*/subsamples*/*/uncorr.pkl.gz')
...
def run_variogram(bd, subbd, perms=1000):
    ...
    # determine paths
    z_path = os.path.join(OUTPUT_DIR, 'SALE_merged', bd, subbd, 'uncorr_z.nii.gz')
...
def run(analysis, bd, subbd, n_iters=10000, use_gpu=True, n_cores=4, 
    ...
    dset_path = os.path.join(OUTPUT_DIR, 'data_merged', bd, subbd_clean, 'dset.pkl.gz')
    try:
    ...
    out_dir = os.path.join(OUTPUT_DIR, f'{analysis}_merged', bd, subbd_clean)
    ### TEMP
...
if __name__ == '__main__':
    ...
    parser.add_argument('analysis', type=str, help='analysis to run', 
        choices=['ALE', 'SALE', 'dSALE', 'k_cluster', 'macm', 'variogram'])
